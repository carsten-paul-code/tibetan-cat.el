...
(defun tibetan-compound-generate-translation-suggestion (lines connectors context)
  "Generate a translation suggestion for LINES based on grammar analysis.
CONNECTORS is the inter-line grammar analysis.
CONTEXT provides tradition-specific terminology preferences."
  (let* ((line-translations '())
         (context-name (plist-get (cdr context) :name))
         (all-vocab (make-hash-table :test 'equal))
         (all-verbs '()))

    ;; Collect vocabulary and verbs from all lines
    (dolist (line-data lines)
      (let* ((tibetan (cdr line-data))
        ...
        ;; Store line translation data
        (push `((line-num . ,line-num)
                (tibetan . ,tibetan)
                (gloss . ,(nreverse line-gloss))
                (verbs . ,verbs))
              line-translations)))

    (setq line-translations (nreverse line-translations))

    ;; Generate the suggestion
    (with-temp-buffer
      (insert "** Translation Suggestion\n\n")
      (insert "Based on vocabulary, grammar, and inter-line connections:\n\n")

      ;; Line-by-line glosses
      (insert "*** Line Glosses\n")
      (let ((line-idx 1))
        (dolist (lt line-translations)
          (let ((gloss (alist-get 'gloss lt))
                (verbs (alist-get 'verbs lt)))
            (insert (format "**** Line %d\n" line-idx))
            ;; Word-by-word
            (when gloss
              (insert "| Tibetan | English |\n")
              (insert "|---------|--------|\n")
              (dolist (pair gloss)
                (insert (format "| %s | %s |\n" (car pair) (cdr pair))))
              (insert "\n"))
            ...
            (when verbs
              (let ((main-verb (car verbs)))
                (when (and main-verb (listp main-verb) (consp (car main-verb)))
                  (let ((lemma (alist-get 'lemma main-verb))
                        (meaning (alist-get 'meaning main-verb)))
                    (insert (format "Main verb: *%s* \"%s\"\n\n"
                                    (or lemma "?")
                                    (or (car (split-string (or meaning "?") "," t)) "?")))))))
            (setq line-idx (1+ line-idx)))))

      ;; Connectors summary for translation
      (when connectors
        (insert "*** Sentence Flow\n")
        (insert "How to connect the lines in English:\n\n")
        (dolist (conn connectors)
          (let ((from (alist-get 'from conn))
            ...
            (insert (format "- Line %d → %d: " from to))
            (cond
             ((eq conn-type 'converb)
              (cond
               ((string= connector "ནས")
                (insert "\"Having [done X], ...\" or \"After [X]-ing, ...\"\n"))
               ((member connector '("ཏེ" "སྟེ" "དེ"))
                (insert "\"[X]-ing, ...\" or \"and then ...\"\n"))
               ((member connector '("ཅིང" "ཞིང" "ཤིང"))
                (insert "\"while [X]-ing\" or list with \"and\"\n"))
             ...
             ((eq conn-type 'conjunction)
              (insert "\"and\" or \"together with\"\n"))
             (t (insert (format "%s\n" conn-type))))))
        (insert "\n"))

      ;; Draft suggestion
      (insert "*** Draft Translation\n")
      (insert "/Based on the above analysis, a possible translation:/\n\n")
      (insert "#+begin_quote\n")
      ;; Build a rough draft from the glosses
      (let ((draft-parts '()))
        (dolist (lt line-translations)
          (let* ((gloss (alist-get 'gloss lt))
                 (verbs (alist-get 'verbs lt))
                 ...
                 (key-words '()))
            ;; Extract key content words (skip particles)
            (dolist (pair gloss)
              (let ((tib (car pair))
                    (eng (cdr pair)))
                (unless (or (string= eng "?")
                            (member tib '("ནི" "ཡང" "གི" "ཀྱི" "འི" "ལ" "ན" "དུ" "སུ" "ར")))
                  (push eng key-words))))
            (setq key-words (nreverse key-words))
            ;; Build line draft
            (let ((line-draft (string-join key-words " ")))
              (when verb-meaning
                (setq line-draft (concat line-draft " [" verb-meaning "]")))
              (push line-draft draft-parts))))
        (insert (string-join (nreverse draft-parts) "\n")))
      (insert "\n#+end_quote\n\n")

      (insert "/Note: This is a rough draft based on word-by-word glossing. ")
      (insert "Refine based on context, commentary, and ")
      (insert (format "%s interpretation./\n" context-name))
